...
import com.marsjiang.mygoogleplay.bean.IC_Card_Pos_bean_item;
import com.marsjiang.mygoogleplay.bean.Send_IC_Card_Pos_Bean;
import com.marsjiang.mygoogleplay.util.ToastUtil;
import com.nineoldandroids.view.ViewHelper;
import com.nineoldandroids.view.ViewPropertyAnimator;

public class IC_Card_Pos_view_Activity extends ActionBarActivity {
	private EditText et_bike_pos;
	private ListView lv_bike;
	private ArrayList<IC_Card_Pos_bean_item> buslist;
	private Send_IC_Card_Pos_Bean send_bike_bean;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		buslist = (ArrayList<IC_Card_Pos_bean_item>) getIntent().getSerializableExtra("BikeList");
		final double latitude = getIntent().getDoubleExtra("latitude", 0);
		final double longitude = getIntent().getDoubleExtra("longitude", 0);

		swiperefreshlayout = (SwipeRefreshLayout) findViewById(R.id.swipe_container_iv_card);
		...
		for (IC_Card_Pos_bean_item getBike_bean_item : buslist) {
		...
		lv_bike.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,long arg3) {
				...
				intent.setClass(IC_Card_Pos_view_Activity.this, RoutePlanActivity.class);
				
				IC_Card_Pos_bean_item ic_card_pos_bean_item = buslist.get(arg2);
				//初始坐标
				...
				System.out.println(latitude+"--77777777777777-----"+longitude);
	...
	class MyBikeAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			IC_Card_Pos_bean_item getbike_bean_item_temp = buslist
					.get(position);

			ViewHolder viewHolder;
			if (convertView == null) {
				viewHolder = new ViewHolder();
				convertView = View.inflate(getApplicationContext(),R.layout.ic_card_items, null);
				viewHolder.tv_ic_card_pos_name = (TextView) convertView.findViewById(R.id.tv_ic_card_pos_name);
			...
			viewHolder.tv_ic_card_pos_name.setText(getbike_bean_item_temp.name);
			viewHolder.tv_distance.setText(getbike_bean_item_temp.diatance);
			ViewHelper.setScaleX(convertView, 0.5f);
			ViewHelper.setScaleY(convertView, 0.5f);

			ViewPropertyAnimator.animate(convertView).scaleX(1.0f)
					.setDuration(500)
					.setInterpolator(new OvershootInterpolator()).start();
			ViewPropertyAnimator.animate(convertView).scaleY(1.0f)
					.setDuration(500)
					.setInterpolator(new OvershootInterpolator()).start();
			return convertView;
		}
	...
	void getDataFromServer(String text_in, double latitude, double longitude) {
		...
		send_bike_bean = new Send_IC_Card_Pos_Bean();
		...
		try {
			 ...
			 */
			params.setBodyEntity(new StringEntity(gson.toJson(send_bike_bean),
					"UTF-8"));
		...
		httpUtils.send(HttpMethod.POST,
				"http://220.180.139.42:8980/SmartBusServer/Main", params,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> string) {
						...
						if (string != null) {
							...
							// get_bikebean =
							// gson.fromJson(string.result,Get_Bike_bean.class);
							...
							try {
								...
								// final ArrayList<GetBike_bean_item> buslist =
								// new ArrayList<GetBike_bean_item>();
								for (int i = 0; i < data.length(); i++) {
									IC_Card_Pos_bean_item getbike_bean_item = new IC_Card_Pos_bean_item();
									getbike_bean_item.icId = data.getJSONArray(
											i).get(0)
											+ "";
									getbike_bean_item.name = (String) data
											.getJSONArray(i).get(1);
									getbike_bean_item.address = (String) data
											.getJSONArray(i).get(2);
									getbike_bean_item.summary = data
											.getJSONArray(i).get(3) + "";
									getbike_bean_item.weidu = data
											.getJSONArray(i).get(4) + "";
									getbike_bean_item.jingdu = data
											.getJSONArray(i).get(5) + "";
									getbike_bean_item.diatance = (String) data
											.getJSONArray(i).get(6);
